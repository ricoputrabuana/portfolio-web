...
export default function Hero() {
  ...
  // ── Auto-open: hero muncul sendiri beberapa detik setelah web dibuka ──
  useEffect(() => {
    const autoOpenTimer = setTimeout(() => {
      setShowDialogue(true);
      setPhase("idle");
      setVisibleWords(0);
    }, 2500); // ganti 2500 (ms) sesuai delay yang kamu mau
  ...
  // ── Foto tinggi: lebih besar di desktop, dikecilin di mobile biar ga makan layar ──
  const imgH = isMobile
    ? Math.min(dims.H * 0.50, 380)
    : Math.min(dims.H * 0.72, 560);
          ...
          >
            ...
            <div
              style={{
                ...
                padding: isMobile ? "0 20px 24px" : "0 40px 56px",
                pointerEvents: "auto",
              }}
            >
              {/* ── Foto Rico ── */}
              <AnimatePresence>
                {isActive && (
                  <motion.div
                    ...
                    style={{ flexShrink: 0, alignSelf: isMobile ? "center" : "flex-end" }}
                  >
                    <img
                      src="/rico.png"
              ...
              <div
                style={{
                  ...
                  height: isMobile ? "auto" : imgH,
                  display: "flex",
                  flexDirection: "column",
                  // Nama muncul lebih ke bawah & geser kiri (desktop only)
                  paddingTop: isMobile ? 4 : imgH * 0.42,
                  paddingBottom: isMobile ? 8 : 24,
                  gap: 0,
                  marginLeft: isMobile ? 0 : -48,
                }}
              >
                {/* ── Nama, divider, & teks: semuanya di-mount bareng begitu isActive,
                     jadi tinggi kolom sudah "fixed" dari awal (kayak desktop) —
                     tiap bagian tinggal fade/slide in di posisinya sendiri, ga
                     dorong-dorong konten lain naik/turun. ── */}
                {isActive && (
                  <>
                    {/* ── Nama + tombol × sejajar ── */}
                    <motion.div
                      ...
                      animate={{
                        ...
                        x: showName ? 0 : (isMobile ? -18 : 0),
                        y: showName ? 0 : (isMobile ? 0 : -8),
                      }}
                    ...
                    {/* ── Divider — melebar dari kiri, ga ubah tinggi kolom ── */}
                    <motion.div
                      key="npc-divider"
                      initial={false}
                      animate={{
                        scaleX: showDivider ? 1 : 0,
                    ...
                    {/* ── Deskripsi word-by-word ──
                         "Ghost" (teks lengkap, invisible) reserve tinggi blok dari
                         awal, teks yang kelihatan di-overlay absolute di atasnya —
                         jadi container ga ikut nambah tinggi tiap kata muncul. */}
